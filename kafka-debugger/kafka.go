...
func KafkaWriter(cfg *clowder.AppConfig, topic string) (*kafka.Writer, error) {
	if cfg.Kafka.Brokers == nil {
		return nil, fmt.Errorf("no kafka brokers found")
	}

	brokers := make([]string, len(cfg.Kafka.Brokers))
	for i := range cfg.Kafka.Brokers {
		brokers[i] = fmt.Sprintf("%v:%d", cfg.Kafka.Brokers[i].Hostname, *cfg.Kafka.Brokers[i].Port)
	}
	...
	if cfg.Kafka.Brokers[0].Authtype != nil {
		log.Println("enabling secure auth...")
		tls := CreateTLSConfig(cfg.Kafka.Brokers[0].Cacert)

		mechanism, err := CreateSaslMechanism(cfg.Kafka.Brokers[0].Sasl)
		if err != nil {
			return nil, fmt.Errorf(`unable to create Kafka producer's Sasl mechanism: %w`, err)
		}

		w.Transport = CreateTransport(mechanism, tls)
	}

	return w, nil
}

func KafkaReader(cfg *clowder.AppConfig, topic string) (*kafka.Reader, error) {
	if cfg.Kafka.Brokers == nil {
		return nil, fmt.Errorf("no kafka brokers found")
	}
	...
	if cfg.Kafka.Brokers[0].Authtype != nil {
		log.Println("enabling secure auth...")
		dialer, err = CreateDialer(&cfg.Kafka.Brokers[0])
		if err != nil {
			return nil, err
		}
	}
...
//////////////////////////////////////////
//////////////////////////////////////////
// This was stolen from sources lmao
//////////////////////////////////////////
//////////////////////////////////////////
